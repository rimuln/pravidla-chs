...
. Stanovený <<#cas-lezeni,časový interval na lezení>> je 6 minut ve všech kolech.
. V momentě, kdy soutěžící vejde do soutěžní zóny u paty lezecké cesty, má <<#cas-priprava,časový interval na přípravu>> 40 vteřin. Nezačne-li soutěžící po vypršení těchto 40 vteřin lézt, musí být ke startu okamžitě vyzván. Jakákoliv další prodleva bude mít za následek disciplinární postih v souladu s částí <<#disciplinarni-rizeni>>. <<#cas-priprava,Časový interval na přípravu>> platí i v případě lezení kol, pro které neplatí <<#podminky-izolace,podmínky izolace>>.
. Každý pokus soutěžícího je považovaný za zahájený a čas (<<#cas-lezeni, časový interval na lezení>>) se začne měřit v momentě, kdy obě chodidla soutěžícího opustí zem. Aby se předešlo pochybnostem, traťový rozhodčí určí, zda pokus začal nebo se jedná o úpravu postavení před startem.
...
. Pokus je považovaný za ukončený a měření času musí být zastaveno:
...
. Stanovená kvóta bude naplněna soutěžícími s nejlepším umístěním z předešlého kola poté co byly výsledky prohlášené za oficiální.
