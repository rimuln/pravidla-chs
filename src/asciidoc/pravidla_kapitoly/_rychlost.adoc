...
.. Každé lano je ovládáno dvěma jističi. Jističi musí stát na takovém místě pod stěnou, aby se vyhnuli nebezpečí úrazu zaviněného pádem soutěžícího, padajícími chyty či jiným vybavením, které mohou nastat v průběhu lezení.
.. Po celou dobu lezeckého pokusu soutěžícího na cestě musí jističi dávat neustále dobrý pozor na jeho postup, aby zajistili, že:
...
. Každý závodník může udělat jeden pokus na každé cestě, kromě:
...
. Pokud je to možné, závodníci by měli mít vyhrazený čas na oficiální trénink v závodních cestách před zahájením kvalifikačních kol. Čas zahájení tréninku oznámí hlavní rozhodčí při technickém meetingu (v případě nutnosti oznámí důvody, proč není oficiální trénink možný).
